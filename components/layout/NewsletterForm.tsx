...
export function NewsletterForm() {
  ...
  async function onSubmit(event: React.FormEvent<HTMLFormElement>) {
    ...
    try {
      ...
      if (response.ok) {
        setStatus({ kind: 'sent', message: 'Obrigada. Recebemos o teu email.' });
        form.reset();
      } else {
        setStatus({
          kind: 'error',
          message: body.message ?? `Não foi possível subscrever. Escreve-nos para ${SITE.email}.`,
        });
      }
    } catch {
      setStatus({
  ...
  return (
    <form className={styles.form} onSubmit={onSubmit} noValidate>
      <label htmlFor={emailId} className="oph-visually-hidden">
        O teu email
      </label>
      <input
        id={emailId}
      ...
      <button type="submit" className={styles.submit} disabled={status.kind === 'sending'}>
        {status.kind === 'sending' ? 'A enviar…' : 'Subscrever'}
      </button>
      <p
        id={statusId}
        role="status"
        aria-live="polite"
        className={[styles.status, status.kind === 'error' ? styles.statusError : undefined]
          .filter(Boolean)
          .join(' ')}
      >
